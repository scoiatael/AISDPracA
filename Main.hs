...
readData = do
  str <- getContents
  return $! parseData $ drop 1 $ lines str

mAdd :: Node -> Maybe [Node] -> Maybe [Node]
mAdd node Nothing = Just [node]
mAdd node (Just nodes) = Just (node:nodes)

createNode :: ProtoNode -> Graph -> Graph
createNode (from_node, node) gr = Map.alter (mAdd node) from_node gr

parseData :: [String] -> Graph
parseData ls = foldl (\m l -> createNode (to3Int l) m) Map.empty ls
